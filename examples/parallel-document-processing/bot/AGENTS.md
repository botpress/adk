...
**This Project**: Uses a deep research workflow with progress tracking.
...
**Demo Deep Research Agent** - A research workflow with progress tracking:

- User starts research via `start_research` tool
- Workflow runs in background, updates progress
- Custom messages update frontend UI
- User can cancel with `stop_research` tool

### Key Files

- `agent.config.ts:14` - API_KEY configuration
- `src/conversations/index.ts:37` - start_research tool
- `src/workflows/index.ts:4` - DeepResearchWorkflow definition
- `src/utils/progress-component.ts` - Progress UI helpers
